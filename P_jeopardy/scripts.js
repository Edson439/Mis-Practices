...
arrowUp.addEventListener('click', () => {
  ...
  if (section === 'film') {
    ...
    if (['vocabulary', 'practice', 'student'].includes(section)) {
      if (sec.index > 0) {
        sec.index--;
        showCard(section);
...
function showSection(sectionToShow) {
  // Solo permitimos mostrar practiceContent y studentContent
  if (sectionToShow !== 'practice' && sectionToShow !== 'student') {
  ...
  document.querySelectorAll('.menu-item').forEach(item => {
    ...
    const img = item.querySelector('img');
    if (img) {
      const originalSrc = img.getAttribute('src').replace('_2.png', '.png');
      img.setAttribute('src', originalSrc);
    }
  });
  ...
  if (activeMenu) {
    ...
    if (img) {
      const hoverSrc = img.getAttribute('src').replace('.png', '_2.png');
      img.setAttribute('src', hoverSrc);
    }
  }
